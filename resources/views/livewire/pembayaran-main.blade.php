<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="p-6 lg:p-8 bg-white border-b border-gray-200 rounded-xl mt-8">
        <h1 class="text-2xl font-medium text-gray-900">
            Pembayaran Menu
        </h1>
        <hr class="mb-6">

        <div data-accordion="collapse"
            data-active-classes="bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
            data-inactive-classes="text-gray-500 dark:text-gray-400">
            <h2>
                <button type="button"
                    class="flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400"
                    aria-expanded="false">
                    <tr
                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <th scope="row"
                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        Pembayaran Pemasangan
                    </th>
                    <td class="px-6 py-4 text-left">
                        <a href="pendaftaran-pelanggan"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                    </td>
                    </tr>
                </button>
            </h2>
            <h2>
                <button type="button"
                    class="flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400"
                    aria-expanded="false">
                    <tr
                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <th scope="row"
                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        Cetak Perjanjian Pelanggan
                    </th>
                    <td class="px-6 py-4 text-left">
                        <a href="pendaftaran-pelanggan"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                    </td>
                </tr>

                </button>
            </h2>
            <h2>
                <button type="button"
                    class="flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400"
                    aria-expanded="false">
                    <span>Cetak Ulang BPBRA</span>
                    <td class="px-6 py-4 text-left">
                        <a href="pendaftaran-pelanggan"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                    </td>
                </button>
            </h2>
            <h2 id="accordion-flush-heading-1">
                <button type="button"
                    class="flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400"
                    data-accordion-target="#accordion-flush-body-1" aria-expanded="false"
                    aria-controls="accordion-flush-body-1">
                    <span>Informasi dan Laporan</span>
                    <svg data-accordion-icon class="w-6 h-6 shrink-0" fill="currentColor" viewBox="0 0 20 20"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd"
                            d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                            clip-rule="evenodd"></path>
                    </svg>
                </button>
            </h2>
            <div id="accordion-flush-body-1" class="hidden" aria-labelledby="accordion-flush-heading-1">
                <div class="py-2 border-gray-200 dark:border-gray-700">
                    <div class="relative overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <tbody>
                                <tr
                                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <th scope="row"
                                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        Laporan Penerimaan Kas/Bank
                                    </th>
                                    <td class="px-6 py-4 text-left">
                                        <a href="pendaftaran-pelanggan"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                                    </td>
                                </tr>
                                <tr
                                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <th scope="row"
                                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        Rekap Penerimaan Kas/Bank
                                    </th>
                                    <td class="px-6 py-4 text-left">
                                        <a href="pendaftaran-pelanggan"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                                    </td>
                                </tr>
                                <tr
                                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <th scope="row"
                                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        Rekap Penerimaan Pembayaran
                                    </th>
                                    <td class="px-6 py-4 text-left">
                                        <a href="pendaftaran-pelanggan"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                                    </td>
                                </tr>
                                <tr
                                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <th scope="row"
                                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        Laporan Penerimaan Pembayaran Bank
                                    </th>
                                    <td class="px-6 py-4 text-left">
                                        <a href="pendaftaran-pelanggan"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                                    </td>
                                </tr>
                                <tr
                                class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    Rekap Penerimaan Pembayaran Bank (New)
                                </th>
                                <td class="px-6 py-4 text-left">
                                    <a href="pendaftaran-pelanggan"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                                </td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
            <h2>
                <button type="button"
                    class="flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400"
                    aria-expanded="false">
                    <tr
                    class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <th scope="row"
                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        Status Pendaftaran
                    </th>
                    <td class="px-6 py-4 text-left">
                        <a href="pendaftaran-pelanggan"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Buka</a>
                    </td>
                    </tr>
                </button>
            </h2>
        </div>
    </div>
</div>
